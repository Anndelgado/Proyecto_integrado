...
// ======================================================
//
// Módulo de Tamizaje Psicológico. Igual que TestService.js, el banco
// de preguntas vive como datos estáticos (no hay tabla "tamizajes" en
// el backend). Lo que SÍ se persiste contra el backend (Postgres) son
// las habilitaciones y los resultados, en las tablas
// "tamizajeHabilitaciones" y "tamizajeResultados".
//
// Preguntas de redacción original, inspiradas en los dominios que
// evalúan instrumentos reconocidos (APGAR familiar, PHQ-9, escalas de
// autoeficacia y de riesgo psicosocial), sin ser transcripciones de
// esos instrumentos.
//
// Reglas de negocio:
// - Un estudiante solo puede presentar un tamizaje si el psicólogo lo
//   habilitó antes (colección "tamizajeHabilitaciones", estado "activa").
// - Al enviar respuestas, la habilitación usada pasa a "inactiva"
//   (es de un solo uso).
// - Si el resultado cae en nivel "Alto", o si el estudiante respondió
//   afirmativamente (valor > 0) alguna pregunta marcada como "critica"
//   (riesgo suicida, autolesiones, abuso), se crea automáticamente una
//   alerta en la colección "alertas" que ya usan Admin y Psicólogo.
//
// ======================================================
...
const CATALOGO_TAMIZAJES = [
    ...
    // ------------------------------------------------------------
    // 1. Escala de Funcionamiento Familiar (inspirada en APGAR familiar)
    // ------------------------------------------------------------
    {
        id: "family",
    ...
    // ------------------------------------------------------------
    // 2. Escala de Estado de Ánimo Adolescente (inspirada en PHQ-9 / Beck)
    // ------------------------------------------------------------
    {
        id: "mood",
    ...
    // ------------------------------------------------------------
    // 3. Escala de Autoeficacia Emocional (inspirada en EADA)
    // ------------------------------------------------------------
    {
        id: "self_efficacy",
    ...
    // ------------------------------------------------------------
    // 4. Escala de Riesgo Psicosocial Escolar
    // ------------------------------------------------------------
    {
...
//======================================================
// Habilitaciones (el psicólogo habilita a un estudiante para
// presentar una prueba puntual antes de que pueda responderla)
//======================================================

export async function getHabilitaciones() {

    const response = await fetch(`${HABILITACIONES_URL}?_sort=fecha&_order=desc`);
...
export async function getHabilitacionesEstudiante(estudianteId) {
    ...
    const response = await fetch(`${HABILITACIONES_URL}?estudianteId=${estudianteId}`);
...
/**
 * Habilita a un estudiante para presentar una prueba. Si ya existía
 * una habilitación previa para ese par (estudiante, prueba), la
...
export async function habilitarTamizaje({ estudianteId, testId, psicologoId }) {
    ...
    const response = await fetch(HABILITACIONES_URL, {
...
async function actualizarHabilitacion(id, cambios) {
    ...
    const response = await fetch(`${HABILITACIONES_URL}/${id}`, {
...
export async function getResultadosEstudiante(estudianteId) {
    ...
    const response = await fetch(`${RESULTADOS_URL}?estudianteId=${estudianteId}&_sort=fecha&_order=desc`);
...
export async function getResultados() {
    ...
    const response = await fetch(`${RESULTADOS_URL}?_sort=fecha&_order=desc`);
...
/**
 * Corrige el cuestionario, guarda el resultado, cierra la
 * habilitación usada y, si corresponde, dispara una alerta
 * automática en el módulo de Alertas.
 *
 * @param {Object} params
 * @param {Object} params.estudiante - Sesión completa del estudiante (id, institucionId, cursoId).
 * @param {Object} params.test - Definición de la prueba (de CATALOGO_TAMIZAJES).
...
export async function guardarResultado({ estudiante, test, respuestas }) {
    ...
    // Alerta automática: nivel Alto, o cualquier pregunta crítica
    // respondida afirmativamente (riesgo suicida, autolesiones, abuso),
    ...
    if (esCritico || clasificacion.nivel === "Alto") {
        ...
        }).catch(() => null); // si falla la alerta, no se bloquea el guardado del resultado
