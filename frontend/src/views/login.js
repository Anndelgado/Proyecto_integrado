...
import { setSession, setToken } from "../session.js";
...
function wireLoginForm(root) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      // La verificación de la contraseña ocurre en el backend; aquí solo
      // se recibe un token y los datos públicos del usuario (sin password).
      const { token, user } = await login(documento, password);
    ...
    fechaRegistro: user.fechaRegistro
      ...
      // err.message ahora trae el motivo real (credenciales inválidas,
      // cuenta pendiente de aprobación, o error de red genérico).
      showError(err.message || "No pudimos conectar con el servidor.");
